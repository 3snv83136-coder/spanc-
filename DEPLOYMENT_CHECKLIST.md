...
- [ ] .env.local.example exists with template
  ...
  - [ ] SPANC_PUBLISH_TOKEN pre-filled
...
API integration:
- [ ] SPANC_API_URL = `https://votre-domaine-spanc.fr`
- [ ] SPANC_PUBLISH_TOKEN = `ruiQvAxSn5VL075j9WfTEL7ftvykTXkEgqBvbfFQfsw`
...
- [ ] Fill form:
  - Type: `Contrôle de bon fonctionnement ANC`
  - Date: today
  - Ville: `Solliès-Pont`
  - Transcription (text): `Contrôle ANC fosse toutes eaux 3 m³ filtre à sable, préfiltre encrassé, vidange à programmer` (or use voice)
...
### Test 6: SPANC Publishing
- [ ] Click "Publier sur le site"
- [ ] Should show "Publishing..." state
- [ ] If SPANC is accessible:
  - Should return success with new URL
  - URL should point to votre-domaine-spanc.fr
- [ ] If SPANC not accessible:
...
- [ ] For SPANC publish: verify SPANC_PUBLISH_TOKEN matches Django config

### SPANC Integration Issues
- [ ] Verify SPANC_API_URL is accessible: curl https://votre-domaine-spanc.fr
- [ ] Check Django backend is running
- [ ] Verify SPANC_PUBLISH_TOKEN matches exactly (check for spaces/typos)
- [ ] Check Django /api/gallery/publish/ endpoint is implemented
- [ ] Monitor server logs on SPANC backend for POST requests
...
- [ ] Can download PDF
- [ ] Can publish to SPANC (or see appropriate error if backend unavailable)
