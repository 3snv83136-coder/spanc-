...
export async function POST(req: NextRequest) {
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || 'contact@votre-domaine-spanc.fr'
  const reviewUrl = process.env.GOOGLE_REVIEW_URL || 'https://www.google.com/maps'
  ...
  const followUps = await Promise.all(
    days.map((d) => {
      ...
      return resend.emails.send({
        from: `Spécialiste SPANC <${fromEmail}>`,
        to: recipient,
...
function relanceSubject(jour: number, prenom: string) {
  ...
  if (jour === 4) return `Un petit avis pour Spécialiste SPANC ?`
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
      <tr><td style="background:linear-gradient(135deg,#0e2a52,#2c5fa8);padding:30px;color:#fff">
        ...
        <p style="margin:6px 0 0;opacity:.85;font-size:13px">Spécialiste SPANC</p>
      </td></tr>
      <tr><td style="padding:30px">
        ...
        <p style="margin-top:30px;font-size:13px;color:#666">Cordialement,<br><strong>${tn}</strong> — Expert en assainissement<br>Spécialiste SPANC</p>
      </td></tr>
      <tr><td style="background:#0e2a52;color:#a0c0ff;padding:18px;text-align:center;font-size:11px">
        Spécialiste SPANC · 07 83 63 68 35 · votre-domaine-spanc.fr
      </td></tr>
    </table>
  </td></tr>
</table>
  ...
  <tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
      ...
      <tr><td style="background:#0e2a52;color:#a0c0ff;padding:14px;text-align:center;font-size:11px">
        Spécialiste SPANC · 07 83 63 68 35
      </td></tr>
    </table>
  </td></tr>
</table>
