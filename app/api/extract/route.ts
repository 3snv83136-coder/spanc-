...
const TYPES = [
  'Contrôle de bon fonctionnement ANC',
  'Diagnostic ANC (vente immobilière)',
  'Contrôle de conception (avant travaux)',
  'Contrôle de réalisation (après travaux)',
  'Contrôle d\'installation neuve',
  'Vidange fosse toutes eaux / septique',
  'Réhabilitation installation ANC',
  'Entretien filière de traitement',
]
...
export async function POST(req: NextRequest) {
  ...
  const prompt = `Tu es un assistant qui extrait des informations structurées depuis la dictée vocale d'un technicien SPANC (assainissement non collectif) du Var (83).
...
Réponds UNIQUEMENT avec ce JSON (sans markdown, sans backticks) :
{
  ...
  try {
    ...
    return NextResponse.json({
      type_intervention: 'Contrôle de bon fonctionnement ANC',
      ville: '',
      code_postal: '',
      adresse: '',
      client_nom: '',
